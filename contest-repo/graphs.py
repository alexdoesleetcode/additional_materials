...
def edgelist_to_adjmatrix(edges: List[List[int]], weighted=False, directed=False):
    matrix = {}
    if weighted:
        for u, v, w in edges:
            if not u in matrix:
...
# Takes an adjacency matrix and a vertex, returns all vertices reachable from it
